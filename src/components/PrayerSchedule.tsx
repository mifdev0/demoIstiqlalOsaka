...
export const PrayerSchedule: React.FC<PrayerScheduleProps> = ({ lang }) => {
  ...
  return (
    <section id="schedule" className="relative z-20 -mt-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      <div className="bg-white rounded-t-3xl rounded-b-xl shadow-2xl p-6 sm:p-8 border border-[#004d2c]/15">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
          {/* Realtime Dual Clock: Analog + Digital Osaka */}
          <div className="flex items-center gap-5 text-center lg:text-left">
            {/* Realtime Analog Clock Display */}
            <div className="relative w-16 h-16 rounded-full bg-[#004d2c] border-2 border-[#d97706] shadow-md flex items-center justify-center shrink-0">
              <div className="absolute top-1 text-[8px] text-[#d97706] font-bold">12</div>
              <div className="absolute right-1 text-[8px] text-[#d97706] font-bold">3</div>
              <div className="absolute bottom-1 text-[8px] text-[#d97706] font-bold">6</div>
              <div className="absolute left-1 text-[8px] text-[#d97706] font-bold">9</div>

              <div
                className="absolute w-1 h-4 bg-white rounded-full origin-bottom bottom-1/2 left-[calc(50%-2px)] shadow-xs transition-transform duration-300"
                style={{ transform: `rotate(${hourAngle}deg)` }}
              />
              <div
                className="absolute w-0.5 h-6 bg-[#d97706] rounded-full origin-bottom bottom-1/2 left-[calc(50%-1px)] shadow-xs transition-transform duration-300"
                style={{ transform: `rotate(${minuteAngle}deg)` }}
              />
              <div
                className="absolute w-[1px] h-7 bg-red-400 rounded-full origin-bottom bottom-1/2 left-[calc(50%-0.5px)] transition-transform duration-100"
                style={{ transform: `rotate(${secondAngle}deg)` }}
              />
              <div className="w-2 h-2 rounded-full bg-[#d97706] z-10" />
            </div>

            {/* Realtime Digital Clock & Dates */}
            <div>
              <h3 className="font-montserrat font-bold text-xl text-[#004d2c] leading-tight">
                {t.prayer.title}
              </h3>

              {/* Digital Time & Hijri String */}
              <div className="flex items-baseline gap-2 mt-1">
                <span className="font-mono font-bold text-2xl text-[#004d2c] tracking-tight">
                  {currentTimeJST ? currentTimeJST : '12:00:00'} <span className="text-xs font-semibold text-[#d97706]">JST</span>
                </span>
                {hijriStr && <span className="text-xs font-bold text-[#d97706] hidden sm:inline">({hijriStr})</span>}
              </div>

              <p className="text-xs text-[#111827]/70 flex items-center gap-1 font-inter mt-0.5">
                <MapPin className="w-3.5 h-3.5 text-[#d97706]" /> Osaka, Japan • Lat 34.6937° N, Long 135.5023° E
              </p>
            </div>
          </div>

          {/* Clean Horizontal Prayer Timeline */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 w-full lg:w-auto">
            {prayerList.map((p) => {
              ...
              return (
                <div
                  key={p.key}
                  className={`flex flex-col items-center justify-center p-3.5 rounded-t-2xl rounded-b-md transition-all relative ${
                    isNext
                ...
                >
                  {isNext && (
                    <span className="absolute -top-2 bg-[#d97706] text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-0.5 shadow-xs">
                      <CheckCircle2 className="w-2.5 h-2.5" /> Next
                    </span>
                  )}
                  <span
                    className={`text-[10px] font-bold uppercase tracking-wider font-inter mb-0.5 ${
